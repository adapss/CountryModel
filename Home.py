...
if __name__ == '__main__':
    ...
    db_cxcn_market_reports = DatabaseConnections().get_MiraLite_Connection()
    db_engine_market_reports = DatabaseConnections().get_MiraWIP_engine()
    ...
    with col1:
        if st.session_state.button: #st.button('Create Country Model'):
           with st.spinner("Loading data..."):
                country_model = Country_Model_Generation( db_cxcn_market_reports, selected_report, selected_base_year)
                ...
                country_model_comparison = CountryModelComparisonTest(share_model, sql_country_model_size, forecast_model, sql_country_model_forecast)
                with col1:
                    st.write("Market Shares")
                    country_model_share_diff = country_model_comparison.market_share_comparison()
                    st.write(country_model_share_diff)
                ...
                with col2:
                    ...
                    st.write(country_model_forecast_diff)
